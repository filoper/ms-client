...
struct OutlinedText {
    ...
    void change_color(Color::Name color) { inner.change_color(color); }

    int16_t width() const { return inner.width(); }
};
...
struct ShadowText {
    ...
    void change_color(Color::Name color) { top.change_color(color); }

    int16_t width() const { return top.width(); }
};
}  // namespace ms