...
void MapTooltip::draw(Point<int16_t> pos) const {
    ...
    if (parent == Tooltip::Parent::MINIMAP && mob_labels->empty()
        && npc_labels->empty()) {
        if (desc_label.empty()) {
            ...
            frame.draw(pos + Point<int16_t>(new_width / 2 + 2, new_height - 7),
                       new_width - 14,
                       new_height - 18);
...
void MapTooltip::set_name(Tooltip::Parent p, std::string n, bool bolded) {
    ...
    if (name.empty()
        || (parent != Tooltip::Parent::WORLDMAP
            && parent != Tooltip::Parent::MINIMAP))
        return;

    name_label = Text(bolded ? Text::Font::A12B : Text::Font::A12M,
                      Text::Alignment::CENTER,
                      Color::Name::WHITE,
                      name);
    name_simple = Text(bolded ? Text::Font::A12B : Text::Font::A12M,
                       Text::Alignment::LEFT,
                       Color::Name::WHITE,
                       name);
...
void MapTooltip::set_desc(std::string d) {
    ...
    desc_label = Text(Text::Font::A12M,
                      Text::Alignment::LEFT,
                      Color::Name::WHITE,
                      description,
                      fillwidth);
    desc_simple = Text(Text::Font::A12M,
                       Text::Alignment::CENTER,
                       Color::Name::WHITE,
                       description,
                       fillwidth);
...
void MapTooltip::set_mapid(int32_t mapid) {
    ...
    for (auto l : life) {
        ...
        if (life_object.first == "m" && m < MAX_LIFE) {
            mob_labels[m] = Text(Text::Font::A12M,
                                 Text::Alignment::LEFT,
            ...
            npc_labels[n] = Text(Text::Font::A12M,
                                 Text::Alignment::LEFT,
                                 Color::Name::MALIBU,
                                 life_object.second);
