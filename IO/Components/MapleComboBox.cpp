...
MapleComboBox::MapleComboBox(Type type,
    ...
    for (auto &option : options)
        option_text.push_back(Text(Text::Font::A12M,
                                   Text::Alignment::LEFT,
                                   Color::Name::BLACK,
                                   option));
    ...
    selected = Text(selected_font,
                    Text::Alignment::LEFT,
                    selected_color,
                    options[selected_index]);

    state = Button::State::NORMAL;
    background = ColorBox(width(),
                          options.size() * HEIGHT,
                          Color::Name::DUSTYGRAY,
                          1.0f);
    rect = ColorBox(width() - 2,
    ...
    Point<int16_t> option_pos =
        Point<int16_t>(position.x(),
                       position.y() + textures[state][0].get_dimensions().y())
        + parentpos;
...
void MapleComboBox::draw(Point<int16_t>) const {
    if (active) {
        ...
        if (pressed) {
            ...
            if (current_shown)
                current_rect.draw(
                    DrawArgument(pos.x() + 1, pos.y() + current_pos + 3));
...
bool MapleComboBox::in_combobox(Point<int16_t> cursorpos) {
    Point<int16_t> lt =
        Point<int16_t>(
            position.x() + 1,
            position.y() + textures[state][0].get_dimensions().y() + 1)
        + parentpos;
    Point<int16_t> rb =
        lt + Point<int16_t>(width() - 2, options.size() * HEIGHT - 2);
