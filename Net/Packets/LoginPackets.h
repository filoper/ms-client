...
public:
    ...
    LoginPacket(const std::string &acc, const std::string &pass) :
        OutPacket(OutPacket::Opcode::LOGIN) {
        std::string volumeSerialNumber =
            Configuration::get().get_vol_serial_num();
...
public:
    ...
    TOSPacket() : OutPacket(OutPacket::Opcode::ACCEPT_TOS) { write_byte(1); }
};
...
public:
    ...
    PlayerLoginPacket(int32_t cid) :
        OutPacket(OutPacket::Opcode::PLAYER_LOGIN) {
        write_int(cid);
    }
};
