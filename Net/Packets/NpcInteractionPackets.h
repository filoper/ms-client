...
public:
    NpcTalkMorePacket(int8_t lastmsg, int8_t response) :
        OutPacket(OutPacket::Opcode::NPC_TALK_MORE) {
        write_byte(lastmsg);
        write_byte(response);
    }
...
public:
    ...
    NpcShopActionPacket(int16_t slot, int32_t itemid, int16_t qty, bool buy) :
        NpcShopActionPacket(buy ? Mode::BUY : Mode::SELL) {
        write_short(slot);
        write_int(itemid);
        write_short(qty);
    }
...
protected:
    ...
    NpcShopActionPacket(Mode mode) :
        OutPacket(OutPacket::Opcode::NPC_SHOP_ACTION) {
        write_byte(mode);
    }
};
}  // namespace ms