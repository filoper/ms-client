...
public:
    RegisterPicPacket(int32_t cid, const std::string &pic) :
        OutPacket(OutPacket::Opcode::REGISTER_PIC) {
        skip(1);
...
public:
    SelectCharPicPacket(const std::string &pic, int32_t cid) :
        OutPacket(OutPacket::Opcode::SELECT_CHAR_PIC) {
        write_string(pic);
        write_int(cid);
        write_hardware_info();
    }
...
public:
    DeleteCharPicPacket(const std::string &pic, int32_t cid) :
        OutPacket(OutPacket::Opcode::DELETE_CHAR) {
        write_string(pic);
        write_int(cid);
    }
};
