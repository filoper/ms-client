...
public:
    ...
    constexpr bool straight() const { return a == b; }
    ...
    Point<T> abs() {
        return { static_cast<T>(std::abs(a)), static_cast<T>(std::abs(b)) };
    }
    ...
    // coordinates
    constexpr Point<T> operator-() const { return { -a, -b }; }

    // Return a point whose coordinates have been added the specified amount
    constexpr Point<T> operator+(T v) const { return { a + v, b + v }; }
