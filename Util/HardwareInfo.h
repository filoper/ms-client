...
public:
    HardwareInfo() {
        ...
        bSuccess = GetVolumeInformation(pLetter, szVolume, MAX_PATH,
        &dwSerialNumber, &dwMaxLen, &dwSystemFlags, szFileSystem, MAX_PATH);

        if (bSuccess)
        {
            sprintf(volumeSerialNumber, "%X%X", HIWORD(dwSerialNumber),
        ...
        into the dwBufLen variable if (GetAdaptersInfo(AdapterInfo, &dwBufLen)
        ...
        GetAdaptersinfo\n"); free(volumeSerialNumber); free(hwid); free(macs);
                return;
            }
        }
        ...
        if (GetAdaptersInfo(AdapterInfo, &dwBufLen) == NO_ERROR)
        {
            ...
            // Technically should look at pAdapterInfo->AddressLength and not
        assume it is 6 sprintf(hwid, "%02X%02X%02X%02X%02X%02X",
            ...
            // Technically should look at pAdapterInfo->AddressLength and not
        assume it is 6 sprintf(macs, "%02X-%02X-%02X-%02X-%02X-%02X",
                pAdapterInfo->Address[0], pAdapterInfo->Address[1],
                pAdapterInfo->Address[2], pAdapterInfo->Address[3],
                pAdapterInfo->Address[4], pAdapterInfo->Address[5]);
