...
class Randomizer {
public:
    bool next_bool() const { return next_int(2) == 1; }

    bool below(float percent) const { return next_real(1.0f) < percent; }

    bool above(float percent) const { return next_real(1.0f) > percent; }
